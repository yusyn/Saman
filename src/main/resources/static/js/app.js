(() => {
  ...
  async function runVerify(side) {
    const isPickup = side === "pickup";
    const btn = $(isPickup ? "#btnVerifyPickup" : "#btnVerifyReturn");
    const status = $(isPickup ? "#pickupVerifyStatus" : "#returnVerifyStatus");
    const otherBtn = $(isPickup ? "#btnVerifyReturn" : "#btnVerifyPickup");
    btn.disabled = true;
    otherBtn.disabled = true;
    status.textContent = "منتظر اثر انگشت روی دستگاه… (حداکثر حدود ۴۰ ثانیه)";
    try {
